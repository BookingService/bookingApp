...
export class FlatController {
  async getFlats(req, res) {
    try {
      ...
      if (isAllowToSmoke && isAllowToSmoke !== "false") {
        requirements.isAllowToSmoke = true;
      }
      if (isParkingAvailable && isParkingAvailable !== "false") {
        requirements.isParkingAvailable = true;
      }
      if (withAnimals && withAnimals !== "false") {
        requirements.withAnimals = true;
      }
      if (withKids && withKids !== "false") {
        requirements.withKids = true;
      }
      if (Number(maxGuests)) {
  ...
  async uploadImage(req, res) {
    upload.array("images", 5)(req, res, (err) => {
      ...
      return res.status(200).json({
        urls: fileNames.map(
          (fileName) => `http://localhost:3001/public/uploads/${fileName}`
        ),
      });
    });
  }
